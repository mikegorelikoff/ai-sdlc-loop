...
Windows users can use the [EXE installer](https://mikegorelikoff.github.io/ai-sdlc-loop/guides/windows-install/).

For a local checkout, use `python3 install.py codex-project`. Claude Code uses `claude-code-project`; another compatible agent uses `agent-project --skills-root .agent/skills`. Immutable `v0.1.1` retains its 17-member inventory; the v0.10.0 source installs the 26-member package described below.
