# Validate and Handoff — ai-sdlc-loop-conventional-commit: Conventional Commit Message
...
Validation:
- PYTHONPYCACHEPREFIX=/tmp/ai-sdlc-harness-pycache python3 -m py_compile skills/ai-sdlc-loop-validation/scripts/validation_plan.py -> passed
...
- Do not stage files or create commits; use `$ai-sdlc-loop-commit-prep` for staging and commit execution.
- Do not invent validation results; use `$ai-sdlc-loop-validation` to choose and run checks.
