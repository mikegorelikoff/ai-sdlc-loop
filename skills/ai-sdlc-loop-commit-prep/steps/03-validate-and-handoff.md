# Validate and Handoff — ai-sdlc-loop-commit-prep: Commit Preparation
...
- Stop and resolve branch state with `$ai-sdlc-loop-branching` when medium or large work is still on `dev`, `main`, `master`, or a branch that does not include the active spec slug.
...
- Do not draft commit message content without `$ai-sdlc-loop-conventional-commit`.
- Do not decide test coverage; use `$ai-sdlc-loop-test-cases`, `$ai-sdlc-loop-validation`, and the `ai-sdlc-loop-verify` stage.
