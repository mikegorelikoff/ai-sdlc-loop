...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise LoopError(f"cannot read valid JSON from {path}: {exc}") from exc
...
def current_spec(root: Path, feature: str) -> dict[str, Any]:
    spec = load_json(state_path(root, feature, "spec.json"))
...
def require_approval(root: Path, feature: str, action: str, subject: str) -> dict[str, Any]:
    receipt = load_json(state_path(root, feature, "approvals", f"{action}.json"))
...
def cmd_specify(args: argparse.Namespace) -> None:
    ...
    atomic_json(state_path(root, feature, "spec.json"), spec)
    atomic_json(state_path(root, feature, "state.json"), {"schema": SCHEMA, "feature": feature, "stage": "specified", "spec_fingerprint": spec["fingerprint"]})
...
def cmd_approve(args: argparse.Namespace) -> None:
    ...
    if args.action == "implement":
        ...
        evidence = load_json(state_path(root, feature, "evidence.json"))
    ...
    atomic_json(state_path(root, feature, "approvals", f"{args.action}.json"), receipt)
...
def cmd_verify(args: argparse.Namespace) -> None:
    ...
    atomic_json(state_path(root, feature, "evidence.json"), evidence)
    atomic_json(state_path(root, feature, "state.json"), {"schema": SCHEMA, "feature": feature, "stage": "verified" if ready else "verification-failed", "spec_fingerprint": spec["fingerprint"], "verified_fingerprint": evidence["verified_fingerprint"], "ready": ready})
...
def cmd_promote(args: argparse.Namespace) -> None:
    ...
    for name in ("state", "evidence"):
        path = state_path(root, feature, f"{name}.json")
        if path.exists():
            payload[name] = load_json(path)
    approvals = {}
    for action in ("implement", "commit"):
        path = state_path(root, feature, "approvals", f"{action}.json")
        if path.exists():
            approvals[action] = load_json(path)
...
def cmd_status(args: argparse.Namespace) -> None:
    root = project_root(args.project_root)
    feature = validate_feature(args.feature)
    result = {"spec": load_json(state_path(root, feature, "spec.json")), "state": load_json(state_path(root, feature, "state.json"))}
