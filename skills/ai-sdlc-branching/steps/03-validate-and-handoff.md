...
- If the branch name matches multiple specs or no spec for medium or large work,
  resolve the active bounded specification with `ai-sdlc-specify` before implementation.
...
- Do not create or update bounded specification artifacts; use `ai-sdlc-specify`.
