# Prepare — ai-sdlc-loop-security-testing: Security Testing
...
Do not use it for general correctness or non-security review. Use `ai-sdlc-loop-validation` or `ai-sdlc-loop-code-review` instead. Do not use it before trust boundaries and protected assets are defined. Use `ai-sdlc-loop-specify` or `ai-sdlc-loop-architecture` instead.
