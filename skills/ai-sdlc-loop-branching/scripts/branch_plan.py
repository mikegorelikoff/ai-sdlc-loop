...
def resolve_base_branch(repo_root: Path | None = None) -> tuple[str, str]:
    ...
    configured = subprocess.run(
        ["git", "config", "--get", "ai-sdlc-loop-orchestrate.baseBranch"], text=True,
        cwd=repo_root, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, check=False,
    ).stdout.strip()
    if configured:
        return configured, "repository config ai-sdlc-loop-orchestrate.baseBranch"
...
def main() -> int:
    ...
    rc = emit_profile_report(
        ...
        write=args.write,
        skill_name="ai-sdlc-loop-branching",
        state_args=args,
    )
    print()
    ...
    if not base_branch:
        print("- Branch creation blocker: declare ai-sdlc-loop-orchestrate.baseBranch or configure origin/HEAD before branching.")
    if status:
