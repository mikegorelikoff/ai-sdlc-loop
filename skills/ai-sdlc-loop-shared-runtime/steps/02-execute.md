# Execute — ai-sdlc-loop-shared-runtime: Portable Helper Dependency
...
- Verify the canonical runtime in a harness source checkout:
  ...
  ```bash
  python3 -m unittest discover -s skills/ai-sdlc-loop-shared-runtime/tests -p 'test*.py' -v
  ```

- Verify every skill-owned test file, including hyphenated package paths:

  ```bash
  python3 skills/ai-sdlc-loop-shared-runtime/scripts/ai_sdlc_test_suite.py \
    --format toon
  ```
...
- Validate all source or installed step manifests without loading their
  ...
  ```bash
  python3 skills/ai-sdlc-loop-shared-runtime/scripts/ai_sdlc_steps.py \
...
- A missing `ai-sdlc-loop-shared-runtime/scripts/` directory, incomplete package, import
...
- Locate `ai-sdlc-loop-shared-runtime/scripts/` under that root.
