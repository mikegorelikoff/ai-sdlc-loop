...
def validate_skills(root: Path, baseline: dict[str, Any]) -> list[str]:
    ...
    for skill in expected:
        ...
        scripts = [] if skill == "ai-sdlc-loop-shared-runtime" else sorted((doc.parent / "scripts").glob("*.py"))
...
def validate_installed_runtime(root: Path) -> list[str]:
    ...
    errors: list[str] = []
    runtime_root = root / "skills" / "ai-sdlc-loop-shared-runtime" / "scripts"
...
def main() -> int:
    ...
    if not errors:
        ...
        if "ai-sdlc-loop-shared-runtime" in baseline.get("required_skill_names", []):
