...
def normalize_skills_root(value: str) -> str:
    ...
    if any(part.casefold() in {".git", ".ai-sdlc-loop-orchestrate"} for part in raw_parts):
        raise InstallError("--skills-root must not overlap .git or .ai-sdlc-loop-orchestrate")
...
def selected_inventory(
    ...
    if "ai-sdlc-loop-shared-runtime" not in names:
        raise InstallError("explicit selection must include ai-sdlc-loop-shared-runtime")
...
def read_existing_install(root: Path) -> tuple[str, list[str], list[str], str | None]:
    ...
    root = root.resolve()
    metadata = root / ".ai-sdlc-loop-orchestrate"
    ...
    if record["inventory"] != ".ai-sdlc-loop-orchestrate/harness-managed-skills.txt":
        raise InstallError("existing install inventory path is invalid")
    if record["lock"] != ".ai-sdlc-loop-orchestrate/harness-install-lock.toon":
...
def _install_locked(
    *,
    ...
    destination_root = root.joinpath(*target.split("/"))
    metadata_root = root / ".ai-sdlc-loop-orchestrate"
    ...
    try:
        ...
        record = {
            "agent": agent,
            "installer": INSTALLER_ID,
            "inventory": ".ai-sdlc-loop-orchestrate/harness-managed-skills.txt",
            "lock": ".ai-sdlc-loop-orchestrate/harness-install-lock.toon",
            "profile": profile,
            "revision": revision,
