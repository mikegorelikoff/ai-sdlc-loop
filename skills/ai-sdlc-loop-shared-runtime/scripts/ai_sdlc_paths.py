...
def repository_root_from_skills_root(skills_root: Path) -> Path:
    ...
    for candidate in resolved.parents:
        record_path = candidate / ".ai-sdlc-loop-orchestrate/harness-install.toon"
    ...
    if resolved.name != "skills":
        raise ValueError(
            "custom skills root must match .ai-sdlc-loop-orchestrate/harness-install.toon"
        )
...
def legacy_context_cache_path(workspace_root: Path, feature: str, skill: str) -> Path:
    ...
    return workspace_root / feature / ".ai-sdlc-loop-orchestrate" / "context" / f"{skill}.toon"
