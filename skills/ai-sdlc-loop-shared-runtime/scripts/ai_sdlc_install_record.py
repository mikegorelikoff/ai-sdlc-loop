...
def validate(record_path: Path, skills_root: Path) -> list[str]:
    ...
    if value["inventory"] != ".ai-sdlc-loop-orchestrate/harness-managed-skills.txt":
        errors.append("install record inventory must be .ai-sdlc-loop-orchestrate/harness-managed-skills.txt")
        return errors
    if value["lock"] != ".ai-sdlc-loop-orchestrate/harness-install-lock.toon":
        errors.append("install record lock must be .ai-sdlc-loop-orchestrate/harness-install-lock.toon")
        return errors
    ...
    if not published:
        ...
        if "ai-sdlc-loop-shared-runtime" not in names:
            errors.append("explicit-skills inventory must include ai-sdlc-loop-shared-runtime")
...
def main() -> int:
    ...
    parser.add_argument("--record", type=Path, default=Path(".ai-sdlc-loop-orchestrate/harness-install.toon"))
