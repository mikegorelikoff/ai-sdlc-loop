...
- After the owner selects the business direction, suggest
  `ai-sdlc-loop-requirements-review` for delivery/testability gaps and
  `ai-sdlc-loop-specify` for the accepted bounded implementation contract.
- Existing accepted requirements that only need review should go directly to
  `ai-sdlc-loop-requirements-review`; do not repeat discovery unnecessarily.
- Specify, Implement and commit keep their existing independent authority
  checks. Do not manufacture approval receipts from this packet.
