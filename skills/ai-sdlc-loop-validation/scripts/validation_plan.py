...
def main() -> int:
    ...
    state_rc = run_state_action(args, "ai-sdlc-loop-validation", "implementation")
    if state_rc:
        return state_rc
    ...
    if any(file.startswith("skills/") for file in files):
        ...
        if "ai-sdlc-loop-shared-runtime" in changed_skills and Path("skills/ai-sdlc-loop-shared-runtime/tests").is_dir():
            commands.append(
