# Validate and Handoff — ai-sdlc-loop-validation: Validation Command Selection
...
```text
Validation:
- command: PYTHONPYCACHEPREFIX=/tmp/ai-sdlc-harness-pycache python3 -m py_compile skills/ai-sdlc-loop-validation/scripts/validation_plan.py
  outcome: passed
  reason: validates changed skill metadata.
- command: python3 .agents/skills/ai-sdlc-loop-shared-runtime/scripts/loop.py status --feature example
  outcome: passed
...
- Do not design acceptance scenarios; use `$ai-sdlc-loop-test-cases` and `ai-sdlc-loop-verify`.
- Do not derive scenario matrices; use `$ai-sdlc-loop-test-cases`.
- Do not review code for findings; use `$ai-sdlc-loop-code-review` or `$ai-sdlc-loop-security-testing`.
