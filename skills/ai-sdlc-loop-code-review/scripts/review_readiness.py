...
def skill_metadata_warnings(files: list[str], full_repo: bool) -> list[str]:
    ...
    for skill in changed_skills:
        ...
        if skill == "ai-sdlc-loop-shared-runtime":
            if changed_python:
                warnings.append(
            ...
            warnings.append(
                ...
                "discover -s skills/ai-sdlc-loop-shared-runtime/tests -p 'test*.py' -v"
            )
            continue
...
def main() -> int:
    ...
    state_rc = run_state_action(args, "ai-sdlc-loop-code-review", "implementation")
    if state_rc:
        return state_rc
